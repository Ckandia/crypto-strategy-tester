...
def stats(trades, equity, start):
    ...
    for x in p:
        if x>0:
            ws+=1; ls=0; mws=max(mws,ws)
        else:
            ls+=1; ws=0; mls=max(mls,ls)

    return {
        "trades":len(p),"wins":len(wins),"losses":len(losses),
        "win_rate":len(wins)/len(p)*100,
        "net_profit":sum(p),"return_pct":sum(p)/start*100,
...
def run_backtest(df, cfg):
    ...
    for i in range(1, len(df)-1):
        ...
        if pos is not None:
            ...
            atr = float(row["atr"]) if row["atr"] == row["atr"] else 0
            ...
            if pos.side=="LONG":
                if low <= pos.stop:
                    ...
                    if not pos.tp1_hit and high >= pos.tp1:
                        ...
                        balance+=net; pos.remaining-=q; pos.tp1_hit=True; pos.stop=pos.entry
                    if pos.tp1_hit and not pos.tp2_hit and high >= pos.tp2:
                        ...
                        balance+=net; pos.remaining-=q; pos.tp2_hit=True
        ...
        if pos is None:
            ...
            if sig:
                ...
                qty=risk/distance
                if qty <= 0 or qty != qty: continue

                if sig.side=="LONG":
                    tp1=entry+distance*cfg.TP1_R; tp2=entry+distance*cfg.TP2_R
                else:
                    tp1=entry-distance*cfg.TP1_R; tp2=entry-distance*cfg.TP2_R

                pos=Position(sig.side,entry,sig.stop_reference,qty,qty,tp1,tp2,False,False,None,nxt["open_time"],sig.score)
                balance-=fees(entry*qty,cfg.FEE_RATE)
                equity.append(balance)
...
def record(pos,row,exit_price,qty,net,reason,cost):
    return {
        "entry_time":pos.entry_time,"exit_time":row["close_time"],"side":pos.side,
        "entry":pos.entry,"exit":exit_price,"qty":qty,"net_pnl":net,
        "exit_reason":reason,"signal_score":pos.score,"fees":cost
    }
